...
class RoutingDecision:
    """Public routing contract returned from ChainWeaverAdapter (maps WisePick ECU → flow)."""
    ...
    @classmethod
    def from_contract(cls, contract: WeaverRouterContract) -> RoutingDecision:
        return cls(
...
class WisePickChainWeaverAdapter:
    ...
    def route(self, user_request: str, *, context: Optional[Dict[str, Any]] = None) -> RoutingDecision:
        """Resolve intent via WisePick only (no ChainWeaver execution)."""
        ecu = self._wp.decide(user_request)
        try:
            contract = self._ecu_to_contract(ecu)
        except UnmappedCapabilityError:
...
class ChainWeaverAdapter(WisePickChainWeaverAdapter):
    """Community-maintained WisePick ↔ ChainWeaver integration entry point."""
